...
//
// PROVIDER FALLBACK (additive): if GEMINI_API_KEY is set in the environment,
// every call tries Google's free-tier Gemini API first, and only falls back
// to Anthropic (using the apiKey/model the caller passed in, as before) if
// Gemini isn't configured or its request fails. This is transparent to every
// caller — the exported function name and signature are unchanged, so none
// of the ~24 files that call callAnthropic() needed to be touched.
...
/**
 * Public entry point used by every AI feature in the app. Tries Gemini
 * first when GEMINI_API_KEY is configured (free tier, no expiring credit);
 * falls back to Anthropic — using the apiKey/model the caller supplied —
 * if Gemini isn't configured or its call fails for any reason. If neither
 * provider is configured, throws a clear combined error.
 */
async function callAnthropic({ apiKey, model, system, messages, temperature, maxTokens, timeoutMs }) {
  if (GEMINI_API_KEY) {
    try {
      ...
      if (!apiKey) {
        // No Anthropic fallback configured — surface the Gemini error as-is.
        throw geminiError;
      }
  ...
  if (!apiKey && !GEMINI_API_KEY) {
    throw new UpstreamAIError('No AI provider is configured on the server (set GEMINI_API_KEY and/or ANTHROPIC_API_KEY)', 500);
  }

  return callAnthropicDirect({ apiKey, model, system, messages, temperature, maxTokens, timeoutMs });
}

module.exports = { callAnthropic, callAnthropicDirect, callGeminiDirect, UpstreamAIError };
